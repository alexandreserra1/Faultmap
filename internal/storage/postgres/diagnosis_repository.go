...
// DiagnosisRepository persiste snapshots completos usando o pool compartilhado.
...
func (repository *DiagnosisRepository) Save(ctx context.Context, diagnosis application.Diagnosis) (bool, error) {
	...
	// Nenhuma linha inserida significa que este diagnóstico já existe. Desfazer
	// aqui, em vez de seguir inserindo findings, é o que torna o snapshot
	// imutável: o registro auditável de uma investigação publicada não pode ser
	// reescrito por uma execução posterior.
	if rowsAffected == 0 {
...
// reduzindo o tempo em que a transação fica aberta.
func prepareDiagnosis(diagnosis application.Diagnosis) (preparedDiagnosis, error) {
	...
	sort.Slice(prepared.findings, func(first, second int) bool {
		return prepared.findings[first].id < prepared.findings[second].id
	})
...
// acusado. O cálculo é o mesmo do backend SQLite de propósito: mudar a
// identidade entre backends faria o mesmo diagnóstico produzir chaves
// diferentes e quebraria a comparação entre dois bancos.
